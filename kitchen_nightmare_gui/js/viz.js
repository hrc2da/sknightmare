class TrackingGraph {
    ...
    initialize = () => {
        ...
        let point_selection = svg.selectAll(".data_points_" + this.id)
            .data(this.data)
            .enter()
            .append("circle")
            .attrs({
        ...
        let path_selection = svg.append("path")
            .data([this.data])
...
class Viz {
    ...
    clear() {
        this.svg.selectAll("*").remove(); // sorry
        for (let metric in this.plots) {
...
function showReport(report) {
    ...
    $('#ratings').append("<p>Average Wait Time: " + report["wait_times"][0].toLocaleString(undefined, {
        maximumFractionDigits: 2
    }) + "</p>");
    $('#ratings').append("<p>Average Group Size: " + report["avg_party_size"].toLocaleString(undefined, {
        maximumFractionDigits: 2
    }) + "</p>");
    $('#finances').append("<h3>Finances</h3>");
    $('#finances').append("<p>Total Revenue: <span class='revenue'>$" + report["revenue"].toLocaleString(undefined, {
        maximumFractionDigits: 2
    }) + "</span></p>");
    $('#finances').append("<p>Upfront Costs: <span class='expenses'>$" + report["upfront_costs"].toLocaleString(undefined, {
        maximumFractionDigits: 2
    }) + "</span></p>");
    $('#finances').append("<p>Staffing, Food, and Upkeep Costs: <span class='expenses'>$" + report["total_overhead"].toLocaleString(undefined, {
        maximumFractionDigits: 2
    }) + "</span></p>");
    ...
    $('#finances').append("<p>Profit: <span class='" + profitability + "'>$" + report["profit"].toLocaleString(undefined, {
        maximumFractionDigits: 2
    }) + "</span></p>");
