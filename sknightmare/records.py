...
class RestaurantDay:
  ...
  def get_avg_wait_time(self):
    if len(self.parties) > 0:
      wait_times = [p.seating_wait.total_seconds()/60.0 for p in self.parties] # currently including parties that left before being seated
  ...
  def get_avg_cook_time(self):
    ...
    '''
    if len(self.parties) > 0:
      cook_times = [np.sum(p.order.cook_times.total_seconds()/60.0) for p in self.parties if p.status >= PartyStatus.ORDERED] # currently including parties that left before being seated
  ...
  def get_avg_total_cook_time(self):
    if len(self.parties) > 0:
      cook_times = [p.order.get_total_cook_time().total_seconds()/60.0 for p in self.parties if p.status >= PartyStatus.ORDERED] # currently including parties that left before being seated
      return np.mean(cook_times), np.std(cook_times)
    else:
      return 0, 0
  ...
  def get_menu_stats(self):
    ...
    raw_menu_stats = {item["name"]:[m for m in meals_served if m.order["name"]==item["name"]] for item in self.menu}
    menu_stats = {}
    ...
    for entry in raw_menu_stats:
      ...
      if quantity > 0:
        ...
        cook_time = np.mean([r.cook_time.total_seconds()/60.0 for r in raw])#,np.std([r.cook_time.total_seconds()/60.0 for r in raw]))
      else:
        quality = 0.0
        cook_time = 0.0
      menu_stats[entry] = {
...
class Ledger:
  ...
  def record_current_day(self):
    self.day_log.put(RestaurantDay(self.parties,self.tables,self.menu))
    self.num_days += 1
    self.reset_day()
